...
public class RedisUtil {
    ...
    public void delete(String... key) {
        if (key != null && key.length > 0) {
            if (key.length == 1) {
                ...
                stringRedisTemplate.delete(CollectionUtil.newArrayList(key));
            }
        }
    }
    ...
    public Set<Object> sGet(String key) {
        try {
            return Collections.singleton(stringRedisTemplate.opsForSet().members(key));
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public long sSet(String key, Object... values) {
        try {
            return stringRedisTemplate.opsForSet().add(key, String.valueOf(values));
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
